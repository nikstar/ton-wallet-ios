...
//        let encryptedKey = try await tonlibWrapper.createKeyWithUserPassword(password, mnemonicPassword: mnemonic)
    ...
    public static func `import`(
        ...
        let encryptedKey = try await tonlibWrapper.importKey(words: words)
        return try Key(
            publicKey: encryptedKey.publicKey,
            encryptedSecretKey: encryptedKey.encryptedSecretKey
        )
    ...
    public func words(
        ...
        let words = try await tonlibWrapper.wordsForKey(
    ...
    public func decryptedSecretKey(
        ...
        let decryptedSecretKey = try await tonlibWrapper.decryptedSecretKeyForKey(
            GTTONKey(publicKey: publicKey, encryptedSecretKey: encryptedSecretKey)
        )
